const ProvidersService = {
  ...
  getById(knex, hcp_id) {
    return knex.from("takecharge_providers").select("*").where("hcp_id", hcp_id).first();
  },
  ...
  updateProvider(knex, hcp_id, newProviderFields) {
    return knex("takecharge_providers").where({ hcp_id }).update(newProviderFields);
  },

  insertFolder(knex, newProvider) {
    return knex
      .insert(newProvider)
